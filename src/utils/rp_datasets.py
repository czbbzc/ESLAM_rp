...
class BaseDataset(Dataset):
    def __init__(self, cfg, args, scale, device='cuda:0', rp=False, rp_file=None):
    ...
    def __getitem__(self, index):
        ...
        if self.rp:
            correspondence = self.correspondence[index]
            correspondence = torch.from_numpy(correspondence)
            return index, color_data.to(self.device), depth_data.to(self.device), pose.to(self.device), correspondence.to(self.device)
...
class Replica(BaseDataset):
    def __init__(self, cfg, args, scale, device='cuda:0', rp=False, rp_file=None
        ...
        super(Replica, self).__init__(cfg, args, scale, device, rp, rp_file)
    ...
    def load_cpd(self,rp,depth_paths,color_paths):
        if not rp:
            return
        for i in range(len(depth_paths)):
            ...
            depth_pre = cv2.imread(depth_paths[pre1], cv2.IMREAD_UNCHANGED)
            depth = depth.astype(np.float32) / self.png_depth_scale
            depth_pre = depth_pre.astype(np.float32) / self.png_depth_scale
...
class ScanNet(BaseDataset):
    def __init__(self, cfg, args, scale, device='cuda:0', rp=False, rp_file=None
        ...
        super(ScanNet, self).__init__(cfg, args, scale, device, rp, rp_file)
        ...
        if rp:
            self.correspondence = np.load("{}/{}".format(self.input_folder,rp_file))
